function impactorTempMeltFuncDarcyStokes(fn,eta_0,E_a)
    %{
    ...
    barrViscPhi = @(nonT,phi) exp(Apar*(T_b./(DT.*nonT+T_t)-1)).*exp(-porViscPar*phi)%.*(1-phi); %%%%New viscosity of solid (1-phi)*mu_s
    ...
    for i = 1:1e9
        ...
        fs = [zeros(Grid.p.Nfx,1); fsVec; zeros(Grid.p.N,1)];  %%%%Needs to be changed
        ...
        viscVec = barrViscPhase(tempVec,phiVec,barrViscPhi)%.*(1-phiVec); %%%% effective solid visc;
        ...
        Zd_vec =  GG * (1-phi)./phi.^mm - 2/3*(1-phi); %dim-less compaction viscosity
        ...
        %vf = vm + () * phiVec.^m .* ( Gp * p + rho_g * grav * h_ice.^3 /(mu_b * kappa_c) %calculate the velocity
