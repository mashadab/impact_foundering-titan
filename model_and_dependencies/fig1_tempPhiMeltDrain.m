...
grZ = 300; % grid resolution in radial direction
d = 20;
Gridp.xmin = 0; Gridp.xmax = Xmax/d; Gridp.Nx = grRes; %radial direction
Gridp.ymin = 0; Gridp.ymax = Ymax/d;  Gridp.Ny = grZ; %vertical direction
...
fp = '../Output/';
inds2 = [100,2000,4000,6000,8000];

inds1 = [100,3000,6000,9000,17100];
...
for i = 1:2
    for j = 1:5
        if i ==1 %Close to Stokes
            load(sprintf("../Output/Shigeru_impact_Wakita-Stokes_eta0_14kc5.6e-16_Ea_50_output_%sC.mat",num2str(inds1(j))))
        end
        if i==2  %Close to Darcy-Stokes
            load(sprintf("../Output/Shigeru_impact_Wakita-Stokes_eta0_14kc5.6e-11_Ea_50_output_%sC.mat",num2str(inds2(j))))
        end
...
%Stokes vs 1e-13 case
%% make plot dimensions
fp = '../Output/';
inds2 = [100,10000,20000,30000,38400];
...
for i = 1:2
    for j = 1:5
        ...
        if i==2  %Close to Darcy-Stokes
            load(sprintf("../Output/Shigeru_impact_Wakita-Stokes_eta0_14kc5.6e-13_Ea_50_output_%sC.mat",num2str(inds2(j))))
        end
        %load([fp fns{i} '/i' num2str(inds(i,j)) '.mat'],'T','phi','tVec');
        TPlot = reshape(T,Grid.p.Ny,Grid.p.Nx)*173+100;
        phiPlot = reshape(phi,Grid.p.Ny,Grid.p.Nx);
