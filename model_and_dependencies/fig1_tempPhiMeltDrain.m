...
inds1 = [1,2500,3000,4500,5500];%Stokes
...
%% make plot dimensions = mid kc = 1e-12 m^2
fp = '../Output/';

inds1 = [1,3000,6000,11000,70000];%Stokes
inds2 = [1,3000,6000,11000,70000];%Stokes
...
for i = 1:2
    for j = 1:5
        ...
        if i==2  %Close to Darcy-Stokes
            load(sprintf("../Output/Simple-test_eta0_14kc1.85e-16_Ea_50_output_%sC.mat",num2str(inds2(j))))
        end
        %load([fp fns{i} '/i' num2str(inds(i,j)) '.mat'],'T','phi','tVec');
        TPlot = reshape(T,Grid.p.Ny,Grid.p.Nx)*173+100;
        phiPlot = reshape(phi,Grid.p.Ny,Grid.p.Nx);
