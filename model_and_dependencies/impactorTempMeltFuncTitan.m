function impactorTempMeltFuncTitan(fn,eta_0,E_a)
    %{
    ...
    elseif all(fn == '03330')
        d = 80*1e3; % ice shell thickness, m
    end
    ...
    ocTh = 2*grRes/5; % make ocean below the ice shell: 1/5 factor being 20% of the ice shell
    Gridp.xmin = 0; Gridp.xmax = 2; Gridp.Nx = grRes; %radial direction
    Gridp.ymin = -ocTh/(2*grRes); Gridp.ymax = 1; Gridp.Ny = 2*grRes+ocTh; %vertical direction
    ...
    %%%%
    % Adding a layer of temperate ice
    TGr_layer   = ones(grRes,Grid.p.Nx);  %melting temperature in the ice
    phiGr_layer = zeros(grRes,Grid.p.Nx);
    %%%%
    ...
    trc1(Y(:)<0.9875) = 0;    %How thick is the organics layer? 1 - 0.975 dimensional units
    ...
    for i = 1:1e9
        ...
        if phiFracRem(end) < termFrac || (i > 1000 && phiFracRem(end) > phiFracRem(end-1)) || i >10000
            % save point
