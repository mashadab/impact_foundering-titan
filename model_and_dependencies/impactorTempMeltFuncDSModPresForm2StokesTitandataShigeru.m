function impactorTempMeltFuncDSModPresForm2StokesTitandataShigeru(fn,eta_0,E_a)
    ...
    XX = linspace(0,Xmax*1e-3,100);  %in km
    YY = linspace(0,Ymax*1e-3,300);  %in km
...
%     phiOc = ones(ocTh,Grid.p.Nx);  %Porosity of ocean, K 
    ...
    TGr = TGr; %Temperature, K
    phiGr = phiGr; %Porosity or melt fraction
    ...
    for i = 1:1e9
        ...
        if phiFracRem(end) < termFrac || (i > 1000 && phiFracRem(end) > phiFracRem(end-1)) || i >100000 %1500 to 5000
            %  point
            save(['Shigeru_impact_' fn '_eta0_' num2str(log10(eta_0)) '_Ea_' num2str(E_a/1e3) '_output_' num2str(i) 'kc' num2str(kc) 'C.mat'],...
...
%             if i<1500
            ...
            tt.TileSpacing = 'none';
            set(gca,'Layer','top')
            saveas(hh,sprintf('../figures/newres_fig%d_imp_kc%d.png',i,kc)); 
            saveas(hh,sprintf('../figures/pdfnewres_fig%d_imp_kc%d.pdf',i,kc)); 
...
%             if i<1500
            ...
            tt.TileSpacing = 'none';
            set(gca,'Layer','top')
            saveas(hhh,sprintf('../figures/organics_newres_fig%d_imp_kc%d.png',i,kc)); 
            saveas(hhh,sprintf('../figures/organics_pdfnewres_fig%d_imp_kc%d.pdf',i,kc));
